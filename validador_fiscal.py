...
def acao_validar_pasta():
    ...
    # construir índice de eventos
    events_index = app_logic.build_events_index(arquivos)
    ...
    for f in arquivos:
        try:
            tree = ET.parse(f)
            root = tree.getroot()
            # identificar se é uma nota (presença de infNFe)
            is_nota = any(app_logic.limpar_tag(el.tag).lower() == 'infnfe' for el in root.iter())
            if is_nota:
                dados = app_logic.extrair_dados_xml(f, tipo, events_index=events_index)
                notas.append(dados)
        except Exception:
            continue

    if notas:
        app_logic.dados_extracao = notas
        # mostrar resumo no text_area
        text_area.delete(1.0, tk.END)
        text_area.insert(tk.END, f"Processados: {len(notas)} notas.\n")
        # mostrar primeiras 5 linhas resumidas
        for d in notas[:5]:
            text_area.insert(tk.END, app_logic.formatar_relatorio_texto(d) + '\n\n')
        btn_excel.config(state="normal")
        btn_pdf.config(state="normal")
    else:
        messagebox.showinfo('Resultado', 'Nenhuma nota (infNFe) encontrada na pasta.')
...
frame_files = tk.Frame(janela)
frame_files.pack(pady=10)
tk.Button(frame_files, text="Selecionar XML", command=acao_validar, bg="#2196F3", fg="white").pack(side='left', padx=6)
tk.Button(frame_files, text="Selecionar Pasta", command=acao_validar_pasta, bg="#1976D2", fg="white").pack(side='left', padx=6)
...
btn_excel = tk.Button(frame_botoes, text="Exportar Excel", command=app_logic.exportar_excel, state="disabled", bg="#4CAF50", fg="#FFFFFF")
btn_excel.pack(side="left", padx=5)

btn_pdf = tk.Button(frame_botoes, text="Exportar PDF", command=app_logic.exportar_pdf, state="disabled", bg="#F44336", fg="#FFFFFF")
